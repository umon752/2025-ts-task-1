...
export const plantI: number = 101;
...
console.log(catKeyName); // 'LargePlant'
...
export function updatePlant(input: PlantBase): Partial<PlantBase> {
...
type UpdateProduct = Partial<Omit<Product, 'id' | 'title'>> &
  Required<Pick<Product, 'id' | 'title'>>;
...
*/
const submitProduct = <T extends 'create' | 'update'>(
  type: T,
  product: T extends 'create' ? CreateProduct : UpdateProduct
): string => {
  if (type === 'create') {
    const p = product as CreateProduct;
    return `新增商品成功：${p.title}`;
  }

  const p = product as UpdateProduct;
  return `更新商品成功：${p.id}`;
};

// function submitProduct<T extends 'create' | 'update'>(
//   type: T,
//   product: T extends 'create' ? CreateProduct : UpdateProduct
// ): string {
//   if (type === 'create') {
